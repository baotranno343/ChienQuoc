...
int main(object me, string arg)
{
	...
	if (sscanf(arg, "%s %d", arg2, diem) == 2)
	{
		...
		tell_user(me, sprintf("%s (%d) đã nhận được %d KNB Khóa", who->get_name(), who->get_number(), diem));
		tell_user(who, sprintf("Bạn đã nhận được %d KNB Khóa", diem));
	}
