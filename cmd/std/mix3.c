...
*/
	...
	else //精炼装备
	{
		...
		make = item[0]->get("make");
		level = item[1]->get_level();
		if (level > 3 && item[1]->get_no_give())
		{
			notify("Linh Thạch Khóa chỉ có thể ép tối đa cấp 3");
			return 1;
		}
		if (make == 0)
			make = "";
...
int mix1(object me, object *item)
{
	...
	if (index == "1")
	{
		if (level > 1)
		{
			notify("Sơ cấp Đoạn thạch chỉ rèn được cao nhất 2 sao.");
			return 1;
		}
	}
	if (index == "2")
	{
		if (level > 3)
		{
			notify("Trung cấp Đoạn thạch chỉ rèn được cao nhất 4 sao.");
			return 1;
		}
	}
	if (index == "3")
	{
		if (level >= 5)
		{
			notify("Cao cấp Đoạn thạch chỉ rèn được cao nhất 5 sao.");
			return 1;
		}
	}
	if (index == "4")
	{
		if (level >= 5)
		{
			notify("Cao cấp Đoạn thạch bị kết dính chỉ rèn được cao nhất 5 sao.");
			return 1;
		}
	}
	if (index == "3" || index == "4")
	{
		if (item[1]->get_no_give())
		{
			notify("Đoạn Thạch Khóa chỉ ép được tối đa 4 sao");
			return 1;
		}
	}
	if (me->get_save("passhanhtrang") && me->get_save("ndmkhtk") < 1)
	...
	else
	{
		...
		if (item[1]->get_forge_index() == "4" && !item[1]->is_khacdinh())
		{
		...
		else if (item[1]->get_forge_index() == "4" && item[1]->is_khacdinh())
		{
			ITEM_EQUIP_D->reduce_equip_prop_123(item[0]);
			write_user(me, ECHO "Thật không may, rèn thất bại. %s của bạn vẫn còn y nguyên !", item[0]->get_name());
		}
		else
		{
			item[0]->delete ("forge");
