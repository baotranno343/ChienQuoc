...
varargs int main(object me, string arg, int server) // server 是否系统端走路，用于 EFFECT_CHAR_DREAM
{
        ...
        string chan, id;
        int z0, x0, y0, z1, x1, y1, z, x, y, dx, dy, d, walk, time2, time_now;
        int busy, p, i, size;
        ...
        // 修改出问题的双倍时间
        /*
        if ( gone_time( me->get_login_time_now)+ me->get_game_time_now+2*3600<me->get_double_time_now)
        {
                me->set_double_time(gone_time( me->get_login_time_now)+ me->get_game_time_now+2*3600);
        }
*/
        z0 = get_z(me);
        x0 = get_x(me);
        y0 = get_y(me);
        time_now = time();
        // if (me->is_store_open() || me->get("cap"))
        // {
        //         if (me->get("cap"))
        //         {
        ...
        if (get_effect(me, EFFECT_CHAR_FAINT) || get_effect(me, EFFECT_CHAR_FAINT_2) || get_effect(me, EFFECT_SLEEP) || get_effect(me, EFFECT_CHAR_NO_MOVE) || get_effect(me, EFFECT_CHAR_FREEZE) || get_effect(me, EFFECT_CHAR_SHUT))
        {
        ...
        // if (me->get_2("gocount.count2")>0) tell_user(me, "time=%d count=%d avg=%d", time_now-me->get_2("gocount.time2"), me->get_2("gocount.count2"), (time_now-me->get_2("gocount.time2"))*100/me->get_2("gocount.count2") );
        //  判断是否走路过快
        if (me->get_2("gocount.time2") && me->get_2("gocount.count2") > 30)
        {
                if ((time_now - me->get_2("gocount.time2")) * 100 / me->get_2("gocount.count2") < 20)
                {
        ...
        if (me->get_2("gocount.count2") > 10)
        {
                ...
                if ((time_now - me->get_2("gocount.time2")) * 100 / me->get_2("gocount.count2") >= 40)
                {
...
*/
        ...
        if (time2 > time_now + 4)
        {
                send_user(me, "%c%w%w%c%w", 0x07, x0, y0, 6, time2 - time_now); // 坐标修正
                return 1;
        }
        ...
        if (get_effect(me, EFFECT_CHAR_SLOW) || (get_effect(me, EFFECT_CHAR_LAZY) || (get_effect(me, EFFECT_CHAR_INVISIBLE) && !get_effect(me, EFFECT_03643) && !me->get_save("no_visible"))) && sizeof(arg) > 1)
                arg = arg[0..0]; // 隐身，移动变慢
        ...
        if (get_effect(me, EFFECT_CHAR_CHAN) // 被人缠住(粘手)
            && stringp(chan = me->get_save("pf.chan")) && sscanf(chan, "%d:(%d,%d)", z1, x1, y1) == 3 && (abs(x1 - x) > 2 || abs(y1 - y) > 2))
        {
        ...
        if (me->get_char_action() != DEFAULT_ACT)
                me->set_char_action(DEFAULT_ACT); // 恢复站立状态(中止小动作)
        ...
        {
                me->set_2("gocount.time2", time_now);
                me->set_2("gocount.count2", 0);
        }
        set_heart_state(me, WALK_STAT);
            ...
            && (!objectp(who = me->get_quest("escort.robber#")) || !who->is_escort_robber())) // 寻找蒙面人
        {
                ...
                if ((p = get_valid_xy(z, x + random(3) - 1, y + random(3) - 1, IS_CHAR_BLOCK)) && !me->get("autoRun"))
                {
                        x = p / 1000;
                        y = p % 1000;
