...
void create()
{
	set_name("Kĩ Năng Bang Hội Ngoại Công");
	set_picid_1(9808);
	set_picid_2(9808);
	set_level(10);
	set_value(50);
...
{
	return "Kĩ năng bang hội \nSau khi sử dụng tất cả kĩ năng Bang hội phái ngoại sẽ đạt cấp 50 và cường hóa đạt 5";
}
...
int get_use_effect_callout(object who, object gift)
{
	...
	who->set_skill(0111, 50);
	who->set_skill(0121, 50);
	who->set_skill(0141, 50);
	who->set_skill(0151, 50);
	who->set_skill(0202, 50);
	who->set_skill(0401, 50);
	who->set_skill(0403, 50);
	who->set_skill(0404, 50);
	who->set_skill(0405, 50);
	send_user(who, "%c%s", ';', "Tất cả kĩ năng " HIR "Bang Hội dành cho hệ Ngoại " NOR "đã được nâng cấp");
	return 0;
}