...
// 宝石锻造的辩别标志
string get_forge_index() { return "6"; }
...
void create()
{
	set_name("Cao Cấp Đoạn Thạch Đặc Biệt");
...
string get_desc()
{
	return HIY "Chỉ có thể luyện trang bị 7 sao trở lên. Giúp trang bị đạt trực tiếp cấp rèn tiếp theo.\n Có thể luyện tối đa 8 sao.";
}
...
int move_item_callout(object me, object item, object equip, int type)
{
	...
	if (equip->is_equip() == 0 && equip->get_real_name() != "Đoạn Thạch")
		return 0;
	if (equip->is_equip())
	{
	...
	if (equip->get_real_name() == "Đoạn Thạch")
	{
		if (item->get_forge_index() != equip->get_forge_index())
			...
			notify("Đoạn Thạch Đặc Biệt không cần hợp thành.");
		return 1;
	}
	...
	if (level < 7)
	{
		notify("Đoạn Thạch Đặc Biệt chỉ rèn được trang bị 7 sao trở lên.");
		return 1;
	}
	if (level >= 8)
	{
		notify("Chỉ có thể rèn được tối đa 8 sao.");
		return 1;
	}
	rate = 10000;
	for (i = 4; i <= level; i++)
	...
	if (type == 1)
	{
		result = sprintf("Trang bị nếu rèn sẽ trực tiếp thăng cấp sao kế tiếp, tỷ lệ thành công là" HIR " %d%%" NOR "，\nNgươi xác định phải rèn %s không?\n" ESC "Chấp nhận\nmix4 0# %x# %x#\n" ESC "Hủy bỏ",
