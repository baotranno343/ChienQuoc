...
int get_item_value() { return 4; }
int get_item_value_2() { return 4; }
...
string get_desc()
{
        return "Bình nhỏ chứa dược thủy rất thần hiệu, khi sử dụng sẽ lập tức hồi phục 800 điểm Pháp Lực. Có thể tái sử dụng sau 2 giây.";
}
...
{
        int type;
        if (get_item_type_2() == ITEM_TYPE_2_ADD_HP)
                type = EFFECT_USE_ITEM_1;
        else
