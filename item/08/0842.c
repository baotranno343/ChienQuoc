...
string get_desc()
{
	return "Song Tử là 1 trong 12 cung Hoàng Đạo. Khi được triệu hồi, sẽ giúp chủ nhân gia tăng thêm 5% Ngoại Kháng, 5% Nội Kháng, 5% Khí Huyết, 5% Bạo Kích.";
}
string get_short()
{
	return "Song Tử là 1 trong 12 cung Hoàng Đạo. Khi được triệu hồi, sẽ giúp chủ nhân gia tăng thêm 5% Ngoại Kháng, 5% Nội Kháng, 5% Khí Huyết, 5% Bạo Kích, Thời gian giới hạn 14 ngày. Nhận được sẽ tự phong ấn.";
}
// 函数：使用效果
...
int get_use_effect_callout(object me, object item)
{
	...
	if (item->get_lock_time() == 0)
		item->set_lock_time(time() + 14 * 24 * 60 * 60); //处理时间控制上面的。
	obj = me->get("zombie");
	size = sizeof(obj);
	if (size)
	{
	...
	{
		send_user(me, "%c%s", '!', "Đã hết hạn sử dụng !");
		return;
	}
	item->set_no_drop(1);
	item->set_no_get(1);
	...
	me->set_2("jg_effect_buff.sz2", 30000);
...
string get_pet_desc()
{
	return "增加主人5%物防、5%法防、5%气血、5%暴击";
}