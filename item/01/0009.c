...
int get_use_effect_callout( object me, object item )
{
	...
	if (item2->get_item_level()!=3)
	{
		send_user( me, "%c%s", '!', "Cấp bậc của Tín Vật không phù hợp" );
		return 0;
	}	
	send_user(me, "%c%c%w%s", ':', 3, 0, "Phượng Vũ Ngọc Bội sau khi sử dụng sẽ biến mất \n Bạn muốn sử dụng không????\n"+ESC+"Chấp nhận\n"+sprintf("use ! %x#\n", getoid(item))+ESC+"Hủy bỏ\nCLOSE\n");
	return 0;
}
