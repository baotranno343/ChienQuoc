...
string get_desc()
{
        return " \n★★★★★★★ \nNgọc bội đại diện cho Thiên Tử, tiềm ẩn ma lực vô cùng lớn.\n Khả năng Tín Vật Môn Phái ở cấp 7 tăng lên cấp 8.\n Người chơi phải đợi đạt đến cấp 55 mới có thể sử dụng ngọc bội.";
}
...
int get_use_effect_callout( object me, object item )
{
	...
	if (item2->get_item_level()!=7)
	{
		send_user( me, "%c%s", '!', "Cấp bậc của Tín Vật không phù hợp" );
		return 0;
	}	
	send_user(me, "%c%c%w%s", ':', 3, 0, "Thiên Ân Ngọc Bội sau khi sử dụng sẽ biến mất \n Bạn muốn sử dụng không???\n"+ESC+"Chấp nhận\n"+sprintf("use ! %x#\n", getoid(item))+ESC+"Hủy bỏ\nCLOSE\n");
	return 0;
}
