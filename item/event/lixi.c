...
string get_desc()
{
	return "Lì xì năm mới, mang lại nhiều điều may mắn và vô cùng ý nghĩa cho mọi người.";
}
...
int get_use_effect_callout(object who, object gift)
{
	...
	if (item)
	{
		...
		p = item->move2(who);
		item->add_to_user(p);
		USER_D->user_channel(sprintf("Bằng hữu " HIR "%s" NOR " mở Hồng Bao nhận được " HIY "%s" NOR ".", who->get_name(), item->get_name()));
	}
	return 0;
}
