...
{ 
	...
	if ( who->get_legend(TASK_49, 21) && !who->get_legend(TASK_49, 22) )
	{
		i = who->add_save("yaodian_daocaoren",1);
		if ( i >= 3 )
		{
			who->set_legend(TASK_49, 22);
	                send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,47,"消灭稻草人(完成)" );
			i = 3;
		}
		send_user(who,"%c%s",'!',sprintf("Bù Nhìn %d/3",i));
		USER_TASK_D->send_task_intro(who,2,TID_NEWBIE_01,47);	
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4722 * rate / 100 ) ;    // NONE

/*              else if( p < 4722 * rate / 100 )    // 金钱
                {
