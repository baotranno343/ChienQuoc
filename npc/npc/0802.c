...
// 函数：获取人物造型
int get_char_picid() { return 1253; }

// 函数：构造处理
void create()
{
        set_name("Địa Ngục Quán Yêu");
...
void drop_items_callout(object me, object who)
{
        ...
        {
                who->add_save_2("soquaicandiet7.score", 1);
                send_user(who, "%c%s", '!', sprintf("Đã giết được %d/20", who->get_save_2("soquaicandiet7.score")));
                if (who->get_save_2("soquaicandiet7.score") > 19)
                {
