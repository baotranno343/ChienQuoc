...
// 函数：获取人物造型
int get_char_picid() { return 0202; }
...
void create()
{
        ...
        NPC_ENERGY_D->init_level(this_object(), 21 + random(3)); //
        set_max_seek(6);
...
void drop_items_callout(object me, object who)
{
        ...
        {
                who->add_save_2("soquaicandiet1.score", 1);
                send_user(who, "%c%s", '!', sprintf("Đã giết được %d/20", who->get_save_2("soquaicandiet1.score")));
                if (who->get_save_2("soquaicandiet1.score") > 19)
                {
        ...
        for (i = 0; i < size; i++)
        {
                ...
                else if (p < 5286 * rate / 100) // 防具(门派)
                {
                        ...
                        case 0:
                                ...
                                break;
                        case 1:
                                file = ARMOR_FILE->get_armor_file_2(10);
                                break;
