...
// 函数：获取人物造型
int get_char_picid() { return 1224; }

// 函数：构造处理
void create()
{
        set_name("Địa Ngục Chu Chu");

        set_char_type(FIGHTER_TYPE); // init_level 要用到

        set_head_color(0x0);

        NPC_ENERGY_D->init_level(this_object(), 110 + random(5)); //
        set_max_seek(4);
...
void drop_items_callout(object me, object who)
{
        ...
        {
                who->add_save_2("soquaicandiet10.score", 1);
                send_user(who, "%c%s", '!', sprintf("Đã giết được %d/20", who->get_save_2("soquaicandiet10.score")));
                if (who->get_save_2("soquaicandiet10.score") > 19)
                {
        ...
        rate = me->correct_drop_rate(me->get_level() - who->get_level()) * who->get_cuthudaotime_rate() / 100;
        ...
        for (i = 0; i < size; i++)
        {
                ...
                else if (p < 8940 * rate / 100) // 武器(门派)
                {
                        ...
                        {
                        case 0:
                                file = WEAPON_FILE->get_weapon_file_2(100);
                                break;
                        case 1:
                                file = WEAPON_FILE->get_weapon_file_2(110);
                                break;
                        case 2:
                                file = WEAPON_FILE->get_weapon_file_2(110);
                                break;
                ...
                else if (p < 9100 * rate / 100) // 防具(门派)
                {
                        ...
                        {
                        case 0:
                                file = ARMOR_FILE->get_armor_file_2(100);
                                break;
                        case 1:
                                file = ARMOR_FILE->get_armor_file_2(110);
                                break;
                        case 2:
                                file = ARMOR_FILE->get_armor_file_2(110);
                                break;
                ...
                else if (p < 9850 * rate / 100) // 杂物２
                {
                        ...
                        {
                        case 0:
                                file = "/item/stuff/0129";
                                break;
                        case 1:
                                file = "/item/stuff/0148";
                                break;
                        case 2:
                                file = "/item/stuff/0053";
                                break;
                        case 3:
                                file = "/item/stuff/0051";
                                break;
