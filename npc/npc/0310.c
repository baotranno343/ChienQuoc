...
// 函数：获取人物造型
int get_char_picid() { return 0300; }
...
void check_legend_task(object who)
{
        ...
        if (who->get_legend(TASK_07, 10) && !who->get_legend(TASK_07, 11))
        {
                if ((i = who->add_save_2("heweixie", 1)) >= 7)
                {
                        send_user(who, "%c%c%c%w%w%s", 0x51, 2, 2, TID_WEIGUO, 7, "梦的困扰(完成)");
                        who->set_save_2("heweixie", 7);
                        who->set_legend(TASK_07, 11);
                }

                USER_TASK_D->send_task_intro(who, 2, TID_WEIGUO, 7);
                send_user(who, "%c%s", '!', sprintf(HIY "褐尾蝎 %d/7", i));
        }
}
// 函数：掉宝奖励
