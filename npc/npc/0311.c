...
void check_legend_task(object who)
{
        ...
        if (who->get_legend(TASK_08, 1) && !who->get_legend(TASK_08, 2) && random(100) < 30)
        {
                if (objectp(item = present("衣服", who, 1, MAX_CARRY)) && item->is_legend_item() == 1)
                {

                        if (item->get_amount() < 6)
                                item->add_amount(1);
                }
                else
                {

                        item = new ("item/98/0032");
                        if (!item)
                                return;
                        p = item->move(who, -1);
                        if (!p)
                ...
                if ((i = who->add_save_2("cgyifu", 1)) >= 6)
                {
                        send_user(who, "%c%c%c%w%w%s", 0x51, 2, 2, TID_CHUGUO, 13, "守卫的衣服(完成)");
                        who->set_save_2("cgyifu", 6);
                        who->set_legend(TASK_08, 2);
                }

                USER_TASK_D->send_task_intro(who, 2, TID_CHUGUO, 13);
                send_user(who, "%c%s", '!', sprintf(HIY "衣服 %d/6", i));
        }
}
// 函数：掉宝奖励
