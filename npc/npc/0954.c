...
void create()
{
        set_name("Thuỷ Hoả Yêu Vương");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x9390);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 95+ random(5) );    // 
        set_max_seek(4);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6000 * rate / 100 ) ;    // NONE

                else if( p < 6300 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6300 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 7200 * rate / 100 )    // 武器(门派)
                {
                        if( equip >= total2 ) continue;

                        switch( random(3) )
                        {
                       case 0 : file = WEAPON_FILE->get_weapon_file_2(95);  break;
                       case 1 : file = WEAPON_FILE->get_weapon_file_2(100);  break;
                       case 2 : file = WEAPON_FILE->get_weapon_file_2(105);  break;
                ...
                else if( p < 8100 * rate / 100 )    // 防具(门派)
                {
                        if( equip >= total2 ) continue;

                        switch( random(3) )
                        {
                       case 0 : file = ARMOR_FILE->get_armor_file_2(95);  break;
                       case 1 : file = ARMOR_FILE->get_armor_file_2(100);  break;
                       case 2 : file = ARMOR_FILE->get_armor_file_2(105);  break;
                ...
                else if( p < 8500 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/quest/product2"->get_random_level_blue_product(95,95);  break;
                ...
                else if( p < 9850 * rate / 100 )    // 杂物３
                {
                       ...
                       case 1 : file = "/item/stuff/0309";  break;
                       case 2 : file = "/item/stuff/0310";  break;
                ...
                else if( p < 10000 * rate / 100 )    // 杂物４
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/08/0015";  break;
