...
// 函数：获取人物造型
int get_char_picid() { return 1242; }

// 函数：构造处理
void create()
{
        set_name("Địa Ngục Ma Binh");

        set_char_type(FIGHTER_TYPE); // init_level 要用到

        set_head_color(0x0);

        NPC_ENERGY_D->init_level(this_object(), 100 + random(5)); //
        set_max_seek(4);
...
void drop_items_callout(object me, object who)
{
        ...
        {
                who->add_save_2("soquaicandiet9.score", 1);
                send_user(who, "%c%s", '!', sprintf("Đã giết được %d/20", who->get_save_2("soquaicandiet9.score")));
                if (who->get_save_2("soquaicandiet9.score") > 19)
                {
