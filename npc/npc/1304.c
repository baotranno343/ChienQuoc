...
// 函数：获取人物造型
int get_char_picid() { return 1244; }

// 函数：构造处理
void create()
{
        set_name("Địa Ngục Cương Thi");

        set_char_type(FIGHTER_TYPE); // init_level 要用到

        set_head_color(0x0);

        NPC_ENERGY_D->init_level(this_object(), 130 + random(5)); //
        set_max_seek(4);
...
void drop_items_callout(object me, object who)
{
        ...
        {
                who->add_save_2("soquaicandiet12.score", 1);
                send_user(who, "%c%s", '!', sprintf("Đã giết được %d/20", who->get_save_2("soquaicandiet12.score")));
                if (who->get_save_2("soquaicandiet12.score") > 19)
                {
        ...
        for (i = 0; i < size; i++)
        {
                ...
                /*             else if( p < 9010 * rate / 100 )    // 武器(基本)
                {
                        ...
                        {
                                item = new( "/item/mtg/phieuthuongnhan" );
                                TEAM_D->drop_item(item,who);
                                item->set_user_id(owner);
