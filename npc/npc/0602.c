...
// 函数：获取人物造型
int get_char_picid() { return 1239; }

// 函数：构造处理
void create()
{
        set_name("Địa Ngục Săn Báo");

        set_char_type(FIGHTER_TYPE); // init_level 要用到

        set_head_color(0x0);

        NPC_ENERGY_D->init_level(this_object(), 60 + random(5)); //
        set_max_seek(4);
...
void drop_items_callout(object me, object who)
{
        ...
        {
                who->add_save_2("soquaicandiet5.score", 1);
                send_user(who, "%c%s", '!', sprintf("Đã giết được %d/20", who->get_save_2("soquaicandiet5.score")));
                if (who->get_save_2("soquaicandiet5.score") > 19)
                {
        ...
        for (i = 0; i < size; i++)
        {
                ...
                else if (p < 8940 * rate / 100) // 武器(门派)
                {
                        ...
                        case 0:
                                ...
                                break;
                        case 1:
                                file = WEAPON_FILE->get_weapon_file_2(65);
                                break;
                        case 2:
                ...
                else if (p < 9100 * rate / 100) // 防具(门派)
                {
                        ...
                        case 0:
                                ...
                                break;
                        case 1:
                                file = ARMOR_FILE->get_armor_file_2(65);
                                break;
                        case 2:
