...
void create()
{
        set_name("Quyền Pháp Sư");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x0);

        NPC_ENERGY_D->init_level( this_object(), 45+ random(5) );    // 
        set_max_seek(4);
...
{
				if(who->add_save_2("soquaicandiet1.score")>=2)
				{
								send_user(who,"%c%s",'!',sprintf("Nhiệm vụ thương nhân đã hoàn thành"));
								return 0;
				}
				who->add_save_2("soquaicandiet1.score",1);
				send_user(who,"%c%s",'!',sprintf("Đã giết được %d/2",who->get_save_2("soquaicandiet1.score")));
}*/
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4961 * rate / 100 ) ;    // NONE

                else if( p < 5211 * rate / 100 )    // 金钱
                {
