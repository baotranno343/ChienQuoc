...
void create()
{
        set_name("Triệu Quốc Đao Thuẫn Binh");
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_16, 8) && !who->get_legend(TASK_16, 9) && who->get_save_2("zgdaobing") < 15)
	{
		if ( (i=who->add_save_2("zgdaobing",1)) >= 15 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,9,"紧缩开支(Hoàn thành)" );
			who->set_save_2("zgdaobing",15);
			who->set_legend(TASK_16, 9);
		}

		USER_TASK_D->send_task_intro(who,2,TID_ZHAOGUO,9);
		send_user(who,"%c%s",'!',sprintf(HIY"Triệu Quốc Đao Thuẫn Binh %d/15",i));		
	}
	if( who->get_legend(TASK_19, 16) && !who->get_legend(TASK_19, 17) && random(100) < 22)
	{
		...
		if ( (i=who->add_save_2("zgnzgmeijiu",1)) >= 5 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,35,"赵国的美酒(Hoàn thành)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_ZHAOGUO,35);
		send_user(who,"%c%s",'!',sprintf(HIY"美酒 %d/5",i));		
	}
	if( who->get_legend(TASK_20, 1) && !who->get_legend(TASK_20, 2) && who->get_save_2("lbwdaodunbing") < 10)
	{
		if ( (i=who->add_save_2("lbwdaodunbing",1)) >= 10 && who->get_save_2("lbwjiandunbing") >= 10 && who->get_save_2("lbwgongbing") >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,39,"清场(Hoàn thành)" );
			who->set_legend(TASK_20, 2);
		}

		USER_TASK_D->send_task_intro(who,2,TID_ZHAOGUO,39);
		send_user(who,"%c%s",'!',sprintf(HIY"Triệu Quốc Đao Thuẫn Binh %d/10",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5414 * rate / 100 ) ;    // NONE

                else if( p < 5664 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 5664 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                        switch( random(4) )
                        {
                       case 0 : file = "/item/32/3283";  break;
