...
// 函数：获取人物造型
int get_char_picid() { return 0551; }

// 函数：构造处理
void create()
{
        set_name("Yên Quốc Kiếm Thuẫn Binh");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x0);

        NPC_ENERGY_D->init_level( this_object(), 56+ random(3) );    // 
        set_max_seek(4);
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_14, 1) && !who->get_legend(TASK_14, 2) && who->get_save_2("bcjdbing") < 10)
	{
		if ( (i=who->add_save_2("bcjdbing",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,23,"报仇雪恨(Hoàn thành)" );
			who->set_save_2("bcjdbing",10);
			who->set_legend(TASK_14, 2);
		}

		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,23);
		send_user(who,"%c%s",'!',sprintf(HIY"Yên Quốc Kiếm Thuẫn Binh %d/10",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5364 * rate / 100 ) ;    // NONE

                else if( p < 5614 * rate / 100 )    // 金钱
                {
