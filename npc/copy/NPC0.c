...
void do_look(object who)
{
	...
	send_user(who, "%c%c%w%s", ':', 3, this_object()->get_char_picid(), sprintf("Khiêu Chiến Danh Tướng：\n Người chơi đạt cấp độ 60 trở lên có thể thử thách phó bản này. Phó bản gồm 10 tầng, mỗi tầng sẽ phải giao đấu với 1 danh tướng Chiến Quốc, nếu thắng lợi sẽ thu về rất nhiều điểm kinh nghiệm, ngân lượng cùng vật phẩm!\n" ESC "Tiến vào Phó bản\ntalk %x# welcome.3\n" ESC "Rời khỏi", id, id, id));
}
...
void do_welcome2(object me, string arg)
{
	...
	case 3:
	{
		...
		for (i = 0; i < size; i++)
		{
			...
			if (team[i]->get_save("kcdt") == iDay)
			{
				result = me->get_name() + "：\n  Bạn đã tham gia phó bản hôm nay rồi!\n";
				send_user(who, "%c%c%w%s", ':', 3, 9115, result);
				return;
			}
		}
		tid = who->get("team_id");
		{
			map = TEAM_D->get_copy_scene(tId, COPY_BARRACK2);
			//		 USER_D->user_channel(sprintf("Bằng hữu "HIY" %s "NOR " cùng đồng đội tiến vào ải  "HIY" 1 "NOR" phá thiên trận.", who->get_name() ));
			if (!map) //创建副本
			{
				map = "sys/copy/barrack2"->new_copy(me, who, 1);
			}
			if (!map)
				return;
			for (i = 0; i < sizeof(team); i++)
			{
				...
				// team[i]->set_2("travel.p", nXy[0] * 1000 + nXy[1]);
