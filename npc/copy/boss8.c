	...
	if (random(100) < rate1) //必掉(有等级修正)
	{
		...
		if (random(100) < 50 * rate1 / 100)
		{
			...
			case 0:
				if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
				{
					item = new ("quest/product2"->get_random_level_yellow_product(97, 107));
					TEAM_D->drop_item(item, who);
					item->set_user_id(owner);
					item->add_to_scene(z, p / 1000, p % 1000);
					item->set("winner", who);
			...
			default:
				if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
				{
					item = new ("quest/product2"->get_random_level_yellow_neck_product(96, 106));
					TEAM_D->drop_item(item, who);
					item->set_user_id(owner);
					item->add_to_scene(z, p / 1000, p % 1000);
					item->set("winner", who);
