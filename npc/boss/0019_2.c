...
int set_fight_status(int level)
{
	...
	set_level(level);
	i = (level - 40) / 5;
	set_max_hp(15000 + 5000 * i);
	set_max_mp(3000 + 1500 * i);
	set_ap(200 + 110 * i);
	set_dp(300 + 130 * i);
	set_cp(500 + 150 * i);
	set_pp(400 + 140 * i);
...
{
	object me = this_object();
	set_name("Kính Ma Khuyển Thần");
	set_head_color(0x4427);
	set_char_picid(0352);
	set_fight_status(45);
	set_walk_speed(3);
	set_attack_speed(15);
...
int perform_action(object who)
{
	...
	if (rate < 3)
	{
		return "skill/04/04244"->perform_action_npc(me, who);
	}
	else if (rate < 25)
	{
		return "skill/04/04233"->perform_action_npc(me, who);
	}
	else if (rate < 30)
	{
		return "skill/04/04211"->perform_action_npc(me, who);
	}
