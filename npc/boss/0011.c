...
void heart_beat_walk()
{
	...
	if (objectp(enemy))
	{
		...
		if (gone_time(saytime) >= 60)
		{
			...
			if (random(100) < 40)
			{
				SAY_CMD->say(me, "来吧来吧，你们知道什么是爱什么是恨吗？");
			}
		}
	}
	else if (RelaxTime == 0)
	...
	if (RelaxTime && gone_time(RelaxTime) >= 120)
	{
		...
		init_fight_status();
		iRelax = 1;
		me->add_to_scene(get_z(me), 166, 180, 4);
	}
...
int perform_action(object who)
{
	...
	if (gone_time(pTime) > 30)
	{
		...
		if (size = sizeof(char))
		{
			SAY_CMD->say(me, "如果一切都能缓慢下来多好");
			"skill/04/04235"->perform_action_npc(me, char[random(size)]);
		}
		return 1;
	}
	...
	if (gone_time(pTime1) > 60)
	{
		...
		{
			SAY_CMD->say(me, "如果一切的一切都不会动了那该多好");
			return skill_04236(me, nTarget[random(size)]);
		}
		return 1;
	}
}
...
{
	object owner, *team, map, item;
	int level, level2, exp, exp2, pot, alllevel;
	int i, size, status, time, flag, p;
	...
	for (i = 0; i < size; i++)
		if (team[i])
		{
			...
			if (team[i]->get_legend(TASK_2_01, 1) && !team[i]->get_legend(TASK_2_01, 2) && USER_INVENTORY_D->count_empty_carry(team[i]) >= 1)
			{
				item = new ("item/98/0240");
				if (!item)
					continue;
				p = item->move(team[i], -1);
				item->add_to_user(p);
				send_user(team[i], "%c%c%c%w%w%s", 0x51, 2, 2, TID_QINGUO, 103, "魂兮归来(完成)");
				team[i]->set_legend(TASK_2_01, 2);
				USER_TASK_D->send_task_intro(team[i], 2, TID_QINGUO, 103);
				send_user(team[i], "%c%s", '!', HIY "三魂六魄 1/1");
				send_user(team[i], "%c%s", ';', "空气中隐隐约约飘来悲凉的语声");
				item->say_hello(1);
			}
		}
}
...
void drop_items(object me, object who)
{
	...
	if (random(100) < 40) //必掉(有等级修正)
	{
		...
		if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
		{
			item = new (WEAPON_FILE->get_suit_file(0, 85, BOOTS_TYPE));
			if (!item)
				return;
			item->set_record(1);
			TEAM_D->drop_item(item, who);
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "哈哈，这就是我的恨……");
		add_hp(2000);
	}
	::stop_fight();
}
