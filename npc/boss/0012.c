...
void win_bonus_callout(object me, object who)
{
	...
	for (i = 0; i < size; i++)
		if (team[i])
		{
			...
			if (team[i]->get_legend(TASK_2_01, 4) && !team[i]->get_legend(TASK_2_01, 5) && USER_INVENTORY_D->count_empty_carry(team[i]) >= 1 && !team[i]->get_save("fb_gsb_jgtsb"))
			{
				TASK_LEGEND_D->give_item(team[i], "item/98/0241", 1);
				send_user(team[i], "%c%s", '!', HIY "机关密图上部 1/1");
				if (team[i]->set_save("fb_gsb_jgtsb", 1) && team[i]->get_save("fb_gsb_jgtzb") && team[i]->get_save("fb_gsb_jgtxb"))
				{
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "哈哈，这就是冲动的惩罚！");
		add_hp(2000);
	}
	::stop_fight();
}
