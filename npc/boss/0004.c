...
void heart_beat_walk()
{
	...
	if (objectp(enemy))
	{
		...
		if (gone_time(saytime) >= 60)
		{
			...
			if (random(100) < 40)
			{
				SAY_CMD->say(me, "Ta làm thịt rượu đến cả Đại Vương cũng tán thưởng, ngươi là cái thá chi !!");
			}
		}
	}
	else if (RelaxTime == 0)
...
int perform_action(object who)
{
	...
	{
		pTime = time();
		SAY_CMD->say(me, "Nhìn lửa này, dùng lửa này nướng đồ ăn nhất định là ngon vô đối !!");
	...
	{
		pTime3 = time();
		SAY_CMD->say(me, "冰镇猪肉能保持肉的鲜味，很好，很好！！");
...
void drop_items(object me, object who)
{
	...
	if (random(100) < 50 * rate1 / 100)
	{

		if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
		{
			item = new (WEAPON_FILE->get_suit_file(0, 55, BOOTS_TYPE));
			if (!item)
				return;
			item->set_record(1);
			TEAM_D->drop_item(item, who);
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "Ha ha ha ... đêm nay lại có thịt mới ăn rồi ??");
		add_hp(2000);
	}
	::stop_fight();
}