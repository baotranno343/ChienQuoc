...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"听说神勇的飞贼王再次出现在地狱魔窟四层！" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" Phi Tặc Vương xuất hiện\n");
}
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "你想得到天恩令吗？先打败我再说！！"); 
						break;
					case 1:
						SAY_CMD->say(me, "天恩令，岂是你这种弱小的人能拿到的！"); 
						break;
					default:
						SAY_CMD->say(me, enemy->get_name()+"，快来接受挑战吧！！"); 
						break;
				}
			}
		}
...
int perform_action( object who ) 
{
	...
	if ( lowhp == 0 && get_hp() * 100 / get_max_hp() < 40 )	//生命值为40％以下
	{
		...
		if ( summon == 0 )
			summon_guard();
		set_lowhp_status();
		SAY_CMD->say(me, "愚蠢的人类！你们必须为激怒本王而付出相应的代价！"); 	
	}
...
//	CHAT_D->rumor_channel( 0, CHAT+"听说凶猛的虎王已经被勇敢的侠士所消灭" ); 
	log_file("boss.txt",short_time()+" Phi Tặc Vương bị tiêu diệt\n");
...
//	if ( rate < rate1 )
	{
	/*	if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//天恩令
		{
		...
		for(i=0;i<10;i++) 	//10酒食
		{
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) ) 
                	{
	                        item = "/sys/party/feizei"->drop_food_wine(80);
	                        if ( !item ) 
	                        	continue;
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
		...
		if ( random(100) < 50)
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//Chiến Quốc Lệnh
		{
		...
		if ( random(100) < 50)
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//小血石
		{
		...
		if ( random(100) < 50)
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//小法石
		{
	...
	if ( !p )
		return;
	rate = random(10000);
	if ( rate < 3000 * rate1 / 100 )	//80级的随机门派金色装备
	{
	...
	else if ( rate < 6000  * rate1 / 100)	//80等级段随机蓝色配方
	{
                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                {
                        item = new("/quest/product2"->get_random_level_blue_product(80,80));
                        TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
                        item->add_to_scene(z, p / 1000, p % 1000);
                        item->set("winner", who);
	...
	else if ( rate < 8500  * rate1 / 100)	//小法石
	{
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	
		{
			item = new( "/item/08/0004" );
	...
	else if ( rate < 9500  * rate1 / 100)	//Tù Và Chiến Quốc
	{
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	
		{
			item = new( "/item/08/0002" );
...
{
	send_user( who, "%c%c%w%s", ':',3,0202,"Phi Tặc Vương:\n    愚蠢的人类！竟敢踏入本王领土！！看来你活得不耐烦了！！\n"+
