...
int perform_action(object who)
{
	...
	if (lowhp == 0 && get_hp() * 100 / get_max_hp() < 40) //生命值为40％以下
	{
		...
		if (summon == 0)
			summon_guard();
		set_lowhp_status();
		SAY_CMD->say(me, "Thần Long Phệ Hỏa!");
	}
...
void win_bonus_callout(object me, object who)
{
	...
	CHAT_D->rumor_channel(0, CHAT + "Nghe nói Vạn Cốt Ma Long đã bị " + who->get_name() + " tiêu diệt!");
	drop_items(me, who);
...
{
	send_user(who, "%c%c%w%s", ':', 3, 8287, "Vạn Cốt Ma Long:\n Có gan tới đây, ắt hẳn không phải chuyện đùa...\n" + sprintf(ESC "Khiêu chiến\ntalk %x# fight\n", getoid(this_object())) + ESC "Rời khỏi");
}
