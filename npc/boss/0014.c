...
int flag;
int saytime, hptime, RelaxTime, pTime, pTime1, pTime2, iRelax, call_npc;
int lowhp, summon;
...
void heart_beat_walk()
{
	...
	if (objectp(enemy))
	{
		...
		if (gone_time(saytime) >= 60)
		{
			...
			if (random(100) < 40)
			{
				SAY_CMD->say(me, "能进入到这里算你本事，但是你在也出不去了！");
			}
		}
	}
	else if (RelaxTime == 0)
	...
	if (RelaxTime && gone_time(RelaxTime) >= 120)
	{
		...
		iRelax = 1;
		call_npc = 0;
		me->add_to_scene(get_z(me), 31, 25, 4);
	}
...
int perform_action(object who)
{
	...
	object me = this_object(), *char, *nTarget, map, npc;

	if (lowhp == 0 && get_hp() * 100 / get_max_hp() < 50) //生命值为50％以下
		lowhp = 1;
	if (lowhp && summon == 0 && (p = get_valid_xy(z = get_z(me), get_x(me), get_y(me), IS_ITEM_BLOCK)))
	{
		...
		if (size = sizeof(nTarget)) //催眠
		{
			for (i = 0; i < size; i++)
			{
				who = nTarget[i];
				if (who->get("anti_dream") || get_effect_3(who, EFFECT_CHAR_DREAM))
					return 2; // 执行成功
		...
		SAY_CMD->say(me, "陵墓总监工，他们实在太强了，我坚持不住了。我需要帮助！");
		//	summon = 1;
		//	npc = new("npc/boss/0015");
		//	npc->set("map",me->get("map"));
		//	npc->add_to_scene(z, p / 1000, p % 1000,1+random(8),9413);
		//	npc->call_help(me);
		return 1;
	}
	...
	{
		pTime = time();
		SAY_CMD->say(me, "闯入者，去死吧！");
		"skill/02/02211"->perform_action_npc(me, who);
		return 1;
	}

	if (gone_time(pTime1) > 15)
	{
		pTime1 = time();
		SAY_CMD->say(me, "身上有那么多外来之力，帮你减少点吧。哈哈");
		"skill/02/02213"->perform_action_npc(me, who);
		return 1;
	}
	if (gone_time(pTime2) > 45)
	{
		pTime2 = time();
		"skill/02/02215"->perform_action_npc(me, who);
		return 1;
	}
}
...
void drop_items(object me, object who)
{
	...
	if (random(100) < 40) //必掉(有等级修正)
	{
		//65级的随机门派套装衣服
		if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
		{
			item = new (WEAPON_FILE->get_suit_file(0, 85, ARMOR_TYPE));
			if (!item)
				return;
			item->set_record(1);
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "没有的闯入者，快快放下手中武器投降吧！");
		add_hp(2000);
	}
	::stop_fight();
}
