...
void taitao(object item, int abc)
{
	...
	case WEAPON_TYPE:
		...
		if (level >= 80)
			pos += 9;
		if (pos >= sizeof(ben_vukhi))
			return;
		if (item->get_weapon_type() == SWORD)
		{
			item->set("ap", ben->get("ap"));
			item->set("cp", ben->get("cp"));
			item->add("cp", ben_vukhi[pos + 1]);
		}
		else
		{
			item->set("cp", ben->get("cp"));
			item->set("ap", ben->get("ap"));
			item->add("ap", ben_vukhi[pos]);
		}
			...
			if (ben_hanglien[pos + i] != 0)
			{
				...
				case 3:
					if (family == 8)
					{
						...
						{
							if (mau == 1)
								item->add("ap", value * 5 / 2);
							else if (mau == 2)
								item->add("ap", value * 5 / 2);
							else if (mau == 5)
								item->add("ap", value * 7 / 2);
						}
					}
					break;
	...
	case WAIST_TYPE:
		...
		if (pos < 0)
			pos = 0;
		if (level >= 80 && family != 7)
			pos += 7;
		if (pos >= sizeof(ben_yeudoi))
			return;
		item->set("hp", ben->get("hp"));
		item->set("mp", ben->get("mp"));
		item->set("dp", ben->get("dp"));
