...
#define PF_NAME "Lạc Tuyết Vô Ngân"

#define SKILL_LEVEL 4
#define TIME_INTERVAL ((me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 20 + 3)
#define SUB_MP (10 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 10 * 2)
#define ADD_EFFECT 25
#define ADD_TIME (10 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 20)
...
string get_desc(object me)
{
        ...
        if (level < SKILL_LEVEL)
                return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 4\n Liên tục trong 15 giây tăng thêm 17%%Thân Thủ.", PF_NAME);
...
{
        int level, interval, interval1, mp;
        object *user;
        ...
        if (gone_time(me->get_perform(PF_START)) < interval)
                return 1; // 小心准备 level
                          //	if( get_effect_3(me, EFFECT_CHAR_MOVE) ) return 1;
                          //   if( gone_time( me->get_perform(PF_START) ) < TIME_INTERVAL ) return 1;

        if (me->get_save("03641"))
        {
                printf(ECHO "Bạn đã có hiệu ứng tăng Thân Thủ rồi!");
                return 1;
        }
        ...
        set_effect(me, EFFECT_CHAR_MOVE, interval1);
