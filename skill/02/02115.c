...
#define TIME_INTERVAL 15 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 26
#define SUB_MP 0
#define SUB_YUAN 2
#define ADD_AP 5 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 26
...
int main(object me, object who, int x, int y, string arg)
{
        ...
        if (me->get_gender() == 2)
                send_user(get_scene_object_2(me, USER_TYPE), "%c%d%d%c%c%c%d", 0x41, getoid(me), time2(), 1, 50, get_d(me), 0); // 9: 施法动作
        else
                send_user(get_scene_object_2(me, USER_TYPE), "%c%d%d%c%c%c%d", 0x41, getoid(me), time2(), 2, 50, get_d(me), 0); // 9: 施法动作
        perform_done(me, 0);
        return 1;
}
        ...
        for (i = 0, size = sizeof(char); i < size; i++)
                if (who = char[i])
                {
                        ...
                        if (who->is_anti_effect())
                                continue;
                        if (get_effect(who, EFFECT_CHAR_CHAN))
                                continue; // 执行成功
...
{
        if (!CHAR_FIGHT_D->attack_action(me, who, 15))
                return 0;
