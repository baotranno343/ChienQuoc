...
#define PF_NAME "Ngự Kiếm Phi Tiên"
...
string get_desc(object me)
{
        ...
        if (level < SKILL_LEVEL)
                return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 45\n Tuyệt kĩ tối cao của Thục Sơn kiếm pháp có thể hợp nhất Tiên Kiếm, tăng thêm 70 điểm Nội Công khi biến hóa thành vô số ảo ảnh sát thương 4 mục tiêu trong tầm nhìn.", PF_NAME);
        cur_level = (level - SKILL_LEVEL) / 15 + 1;
        size = 9 + (level / 80) * 7;
        result = sprintf(HIC " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Tuyệt kĩ tối cao của Thục Sơn kiếm pháp có thể hợp nhất Tiên Kiếm, tăng thêm %d điểm Nội Công khi biến hóa thành vô số ảo ảnh sát thương %d mục tiêu trong tầm nhìn.\n " HIC "Cấp tiếp theo:\n " NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Nội Công，Biến hoá thành %d ảo ảnh sát thương mục tiêu. ",
...
int main(object me, object who, int x, int y, string arg)
{
        ...
        char = get_range_object(z, x0, y0, 8, USER_TYPE) + get_range_object(z, x0, y0, 8, CHAR_TYPE); // 7 x 7 随机
        ...
        if (size > size2)
        {
                size = size - size2;
                for (i = 0; i < size; i++)
                        char2 -= ({char2[random(sizeof(char2))]});
        }

        me->set("pf.02316", char2);
        f1 = me->get_armor_code();
        f2 = me->get_head_code();
        f3 = me->get_weapon_code_2();
        f4 = me->get_reincarnation();
        act = 1;

        for (i = 0, size = sizeof(char2); i < size; i++)
                if (who = char2[i])
                {
                        x = get_x(who);
                        y = get_y(who);
...
int cast_done(object me)
{
        ...
        if (!stringp(arg = me->get_cast_arg()) || sscanf(arg, "%d", level) != 1)
                return 1;

        if (!arrayp(char = me->get("pf.02316")))
                return 2; // 执行成功

        for (i = 0, size = sizeof(char); i < size; i++)
                if (who = char[i])
                {
                        CHAR_FIGHT_D->cast_done(me, who, ADD_CP * 3);
                        if (who && !who->is_die())
