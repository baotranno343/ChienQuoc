...
string get_desc(object me)
{
        ...
        result = sprintf(HIC " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Nguyên khí tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Côn pháp tung hoàng chiến trường như chốn không người, tăng thêm %d điểm Ngoại Công và sát thương 8 hướng xung quanh bản thân.\n " HIC "Cấp tiếp theo:\n " NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Ngoại Công.",
...
int main(object me, object who, int x, int y, string arg)
{
        ...
        if ((level = me->get_skill(THIS_SKILL)) < SKILL_LEVEL)
                return 1;
        if (gone_time(me->get_perform(PF_FLAG)) < TIME_INTERVAL)
                return 1; // 小心准备 me
...
int perform_done(object me, object who)
{
        ...
        if (me->get_skill(THIS_SKILL) >= 61)
                char = get_range_object_2(me, 3, USER_TYPE) + get_range_object_2(me, 3, CHAR_TYPE) - ({me});
        else
...
{
        if (!CHAR_FIGHT_D->attack_action(me, who, 2040))
                return 0;
