...
#define THIS_PERFORM 02315
#define PF_START "02315"
#define PF_NAME "Vân Vũ Tiêu Diêu"
...
string get_desc(object me)
{
        ...
        result = sprintf(HIC " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Kiếm chiêu như mưa gió mù mịt bủa vây, tăng thêm %d điểm Nội Công khi sát thương và làm mục tiêu bị Chậm Chạp trong 6 giây.\n " HIC "Cấp tiếp theo:\n " NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Nội Công.",
...
int main(object me, object who, int x, int y, string arg)
{
        ...
        else
        {
                ...
                if (!objectp(who = get_xy_object(z, x, y, CHAR_TYPE)) && !objectp(who = get_xy_object(z, x, y, USER_TYPE)))
                {
                        ...
                        send_user(get_scene_object_2(me, USER_TYPE), "%c%d%d%c%c%c%d", 0x41, getoid(me), time2(), 1, 50, get_d(me), 0);
        ...
        if (me->get_gender() == 2)
                ...
                send_user(get_scene_object_2(me, USER_TYPE), "%c%d%d%c%c%c%d", 0x41, getoid(me), time2(), 2, 50, get_d(me), 0);
...
int cast_done(object me)
{
        ...
        if (!who->can_be_fighted(me) || !me->can_fight(who))
                return 2; // 执行成功
        send_user(get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 23156, 1, OVER_BODY, PF_ONCE);
        ...
        {
                if (get_effect(who, EFFECT_CHAR_SLOW) || get_effect(who, EFFECT_CHAR_SLOW_0))
                        return 2; // 执行成功
...
void can_perform(object me)
{
        ...
        if (me->get_gender() == 1)
                send_user(me, "%c%w%w%c%c%c%c%c%c", 0x64, THIS_PERFORM, SUB_MP, 1, 10, 2, 50, 6, 0);
        else
                send_user(me, "%c%w%w%c%c%c%c%c%c", 0x64, THIS_PERFORM, SUB_MP, 1, 10, 1, 50, 6, 0);
}