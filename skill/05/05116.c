...
#define HP_PHANTRAM (me->get_skill(THIS_SKILL) / 20)
#define SKILL_LEVEL 42
#define TIME_INTERVAL 3
#define SUB_MP (120 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 14 * 36)
#define ADD_EFFECT (1100 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 14 * 290)
...
string get_desc(object me)
{
        ...
        if (level < SKILL_LEVEL)
                return HIR + PF_NAME + "\n Võ công yêu cầu: 42 cấp\n    Y thuật cao cấp, có thể hồi phục 1100 điểm Khí Huyết cho đồng đội";
        cur_level = (level - SKILL_LEVEL) / 14 + 1;
        result = sprintf(HIC " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Y thuật cao cấp, có thể hồi phục %d điểm Khí Huyết với  %.2f%% khí huyết tối đa của mục tiêu.\n " HIC "Cấp tiếp theo:\n " NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Y thuật cao cấp, có thể hồi phục %d điểm Khí Huyết cho đồng đội.",
                         PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT, to_float(HP_PHANTRAM), cur_level * 14 + SKILL_LEVEL, ADD_EFFECT + 250);
        return result;
}
...
int cast_done(object me)
{
        ...
        hp1 += who->get_max_hp() * HP_PHANTRAM / 100;
        ...
        if (objectp(item = me->get_equip(WEAPON_TYPE)) && item->get_name() == "Lục Tiên Kiếm")
                hp1 += 50 + random(51);
        if (objectp(item = me->get_equip(WEAPON_TYPE)) && item->get_name() == "Phá Pháp Chi Kiếm")
                hp1 += 400 + random(101);
        if (get_effect(who, EFFECT_CHAR_GOOD_BAD))
                hp1 /= 2;
        hp1 += me->get("add_magic_heal");
        hp1 += hp1 * me->get("trilieu") / 100;
