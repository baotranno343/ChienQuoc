...
#define THIS_PERFORM    05117
#define PF_START       "05117"
#define PF_NAME        "Bồ Đề Trị"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   1
#define SUB_MP        ( 10 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 36 )
#define ADD_EFFECT    ( 10000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 250 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 42 cấp\n    Y thuật cao cấp, có thể hồi phục 10000 điểm Khí Huyết cho đồng đội";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Y thuật cao cấp, có thể hồi phục %d điểm Khí Huyết cho đồng đội。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Y thuật cao cấp, có thể hồi phục %d điểm Khí Huyết cho đồng đội。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT, cur_level*150+SKILL_LEVEL, ADD_EFFECT+250 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("05117")==0) return 1;
...
int cast_done( object me )
{
	...
	if( objectp(item=me->get_equip(WEAPON_TYPE)) && item->get_name()=="Ngọc Thần Bảo Kiếm" )
		hp1 += random(51);
	if( objectp(item=me->get_equip(WEAPON_TYPE)) && item->get_name()=="Lục Tiên Kiêm" )
		hp1 += 50 + random(51);
