...
// 函数：获取技能ID
int get_id() { return 74107; }
// 函数：获取技能类型
string get_name() { return "Miễn Nhiễm Chậm Chạp"; }
// 函数：获取技能类型（0、被动；1、主动）
...
{
	return HIC+get_name()+"\n Kỹ năng của Bảo thú, không bị trúng trạng thái chậm chạp.";
}
// 函数：获取指令类型
...
{
	pet->set("anti_slow",1);	//抗 EFFECT_CHAR_SLOW
}
//删除技能的影响
void remove_skill_effect(object pet,object owner)
{
	pet->delete("anti_slow");
}