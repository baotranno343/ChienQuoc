...
#include <action.h>
#include <effect.h>
#define PF_START       "74137"
#define TIME_INTERVAL   20

// 函数：宠物技能
int is_pet_skill() { return 1; }
// 函数：获取技能ID
int get_id() { return 74137; }
// 函数：获取技能类型
string get_name() { return "Thiên Cân"; }
// 函数：获取技能类型（0、被动；1、主动）
...
{
	return HIC+get_name()+"\n Kỹ năng của Bảo thú, Sử dụng có thể làm mục tiêu bị hôn mê 3 giây, kĩ năng tái sử dụng 20 giây \nvà không chồng hiệu quả";
}
// 函数：获取指令类型
...
int cast_done_npc( object me )
{
	...
	user = get_scene_object_2(who, USER_TYPE);
        send_user( user, "%c%d%w%c%c%w%c%c%w%c%c%c", 0x6f, getoid(who), 
