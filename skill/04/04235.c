...
#define SUB_MP (10 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 10 * 8)
#define ADD_AP (200 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 10 * 15)
#define ADD_INTERVAL 5
...
string get_desc(object me)
{
        ...
        if (level < SKILL_LEVEL)
                return HIR + PF_NAME + "\n Võ công yêu cầu: 27 cấp\n Vật phẩm tiêu hao: 3 lá Bùa Giấy\n    Triệu hồi Thương Long tấn công kẻ địch, tăng thêm 200 điểm Nội Công khi sát thương đồng thời làm mục tiêu bị Đóng Băng trong 5 giây";
        cur_level = (level - SKILL_LEVEL) / 10 + 1;
        result = sprintf(HIC " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 3 lá Bùa Giấy\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Triệu hồi Thương Long tấn công kẻ địch, tăng thêm %d điểm Nội Công khi sát thương đồng thời làm mục tiêu bị Đóng Băng trong 5 giây.\n " HIC "Cấp tiếp theo:\n " NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Nội Công，",
...
int cast_done_npc(object me)
{
        ...
        if (ret > 0)
        {
                if (get_effect(who, EFFECT_CHAR_LAZY) || get_effect(who, EFFECT_CHAR_LAZY_0))
                        return 2;                                // 执行成功
                send_user(who, "%c%c%c", 0x3d, 241, 1);          // 更改走路步数
                set_effect(who, EFFECT_CHAR_LAZY, ADD_INTERVAL); // 小心准备 me
                send_user(who, "%c%w%w%c", 0x81, 9013, get_effect_3(who, EFFECT_CHAR_LAZY), EFFECT_BAD);
