...
#define THIS_PERFORM    04139
#define PF_START       "04139"
#define PF_NAME        "Triệu Hồi Thánh Nhân"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 60 cấp\n Vật phẩm tiêu hao: 1 Thiên Hạp\n    Triệu hồi Thánh Nhân hỗ trợ chiến đấu. Khi tấn công Thánh Nhân có khả năng làm địch thủ bị Trọng Thương.";
        cur_level = (level - SKILL_LEVEL)/10 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 1 Thiên Hạp\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Triệu hồi Thánh Nhân hỗ trợ chiến đấu. Khi tấn công Thánh Nhân có khả năng làm địch thủ bị Trọng Thương.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng Thánh Nhân thêm 1000 điểm Khí Huyết:\n    Tăng Thánh Nhân thêm 55 điểm Ngoại Công:\n    Tăng Thánh Nhân thêm 150 điểm Ngoại Kháng\n    Tăng Thánh Nhân thêm 150 điểm Nội Kháng:\n    Tăng Thánh Nhân thêm 40 điểm Thân Thủ.",
...
int main( object me, object who, int x, int y, string arg )
{
        ...
        {
                printf( ECHO "Bạn đã có gọi Cương Thi/Thánh Nhân /Quỷ Tốt" );
                return 1;
        }

        if( !objectp( item = present("Thiên Hạp", me, 1, MAX_CARRY*4) ) )
        {
                printf( ECHO "Phải có Thiên Hạp mới có thể sử dụng \"" PF_NAME "\"." );
                return 1;
        }
...
int cast_done( object me )
{
        ...
        level = to_int(arg);
        send_user(me, "%c%c%w%s", 0x5a, 0, 6, "Gọi Thánh Nhân……");	                
...
/*
        ...
        if( p = get_valid_xy(z, x0 + random(3) - 1, y0 + random(3) - 1, IS_CHAR_BLOCK) )
        {
                ...
                npc->set_name( sprintf( "Thánh Nhân ( %s )", me->get_name() ) );
...
int cast_done_npc( object me )
{
        ...
        if( p = get_valid_xy(z, x0 + random(3) - 1, y0 + random(3) - 1, IS_CHAR_BLOCK) )
        {
                npc = new( "/npc/std/4139" );
//              npc->set_name( sprintf( "Thánh Nhân (%d)", me->get_number() ) );
//              npc->set_level(1);
                npc->set_max_hp( INIT_HP );    // 小心准备 me
                npc->set_ap( INIT_AP );
                npc->set_dp( INIT_DP );
...
void can_perform( object me )
{
	...
	send_user(me, "%c%w%w%c%c%c%c%c%s", 0x64, THIS_PERFORM, SUB_MP, 0, 1, MAGIC_ACT_BEGIN, 0, 9, "Thiên Hạp");
}
