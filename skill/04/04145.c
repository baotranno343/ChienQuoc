...
#define MY_CP (me->get_cp() * 30 / 100)
#define WHO_HP_DOWN (me->get_skill(THIS_SKILL) / 10)
#define SUB_MP (30 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 10 * 15)
#define ADD_EFFECT (MY_CP + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 10 * 200)

#define ADD_INTERVAL 10
...
string get_desc(object me)
{
        ...
        if (level < SKILL_LEVEL)
                return HIR + PF_NAME + "\n Võ công yêu cầu: 22 cấp\    Tà thuật ăn mòn thân thể, làm cho đối thủ liên tục trong vòng 10 giây mất đi Khí Huyết.";
        cur_level = (level - SKILL_LEVEL) / 10 + 1;
        result = sprintf(HIC " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tà thuật ăn mòn thân thể, làm cho đối thủ liên tục trong vòng 10 giây mất đi %d điểm Khí Huyết. + 30%% nội công của bản thân + %d %% máu của đối thủ\n " HIC "Cấp tiếp theo:\n " NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Làm cho đối thủ liên tục trong vòng 20 giây mất đi %d điểm Khí Huyết. ",
                         PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, (ADD_EFFECT - MY_CP), WHO_HP_DOWN, cur_level * 10 + SKILL_LEVEL, ADD_EFFECT + 200);
        return result;
}
...
int cast_done(object me)
{
        ...
        if (get_effect(who, EFFECT_CHAR_HALFGOD))
                return 2; // 处于神打状态下不成功

        if (get_effect_3(who, EFFECT_CHAR_ARMORFALL))
                flag += 1;
        if (get_effect_3(who, EFFECT_MP_FALL))
                flag += 1;
        if (get_effect_3(who, EFFECT_CHAR_POWERFALL))
                flag += 1;
        //        if (flag>=2) return 2;
        damage = ADD_EFFECT;
        if (who->is_user())
        { //50000 * 13/100
                damage += who->get_max_hp() * WHO_HP_DOWN / 100;
        }
        if (-who->get("pf.fall") > damage / (ADD_INTERVAL / 2))
                return 2;
        if (!who->get_max_damage() && who->get_action_mode() == 1)
                who->init_enemy_damage(me, damage);
        else if (damage >= who->get_max_damage() && who->get_action_mode() != 0)
        ...
        who->set("pf.fall", -damage / (ADD_INTERVAL / 2)); // 小心准备 me
        if (who->is_user())
                who->set_save_2("pf.fall", -damage / (ADD_INTERVAL / 2));
...
void can_perform(object me)
{
        ...
        if (interval < 0)
                ...
                send_user(me, "%c%w%c%w%w%s", 0x50, THIS_PERFORM, get_perform_type(), 2, interval, name);
