...
#define ADD_EFFECT 150 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 10 * 200
...
string get_desc(object me)
{
        ...
        if (level < SKILL_LEVEL)
                return HIR + PF_NAME + "\n Võ công yêu cầu: 10 cấp\n    Dùng nội lực hộ thể, chuyển hóa 150 điểm sát thương thành tiêu hao Pháp Lực. Mỗi 1 điểm sát thương tương ứng 1 điểm Pháp Lực tiêu hao. Tâm pháp có thể duy trì trong 5 phút";
...
int main(object me, object who, int x, int y, string arg)
{
        ...
        mpp = ADD_EFFECT;
        if (objectp(item1 = me->get_equip(WEAPON_TYPE)) && item1->get_name() == "Thủy Long Kiếm")
                mpp += 400 + random(101);
        if (objectp(item1 = me->get_equip(WEAPON_TYPE)) && item1->get_name() == "Thanh Long Đao")
                mpp += 400 + random(101);
