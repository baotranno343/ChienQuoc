...
#define THIS_PERFORM    04232
#define PF_START       "04232"
#define PF_NAME        "Ngự Lôi Thuật"

#define SKILL_LEVEL     12
#define TIME_INTERVAL   2
#define SUB_MP          ( 20 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 10 * 8 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 12 cấp\n Vật phẩm tiêu hao: 1 lá Bùa Giấy\n    Hô gọi Ngự Lôi Thiên Sư tấn công địch thủ, tăng thêm 200 điểm Nội Công khi sát thương và làm mục tiêu bị Choáng trong 2 giây";
        cur_level = (level - SKILL_LEVEL)/10 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 1 lá Bùa Giấy\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Hô gọi Ngự Lôi Thiên Sư tấn công địch thủ, tăng thêm %d điểm Nội Công khi sát thương và làm mục tiêu bị Choáng trong 1 giây\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Nội Công.", 
...
int cast_done( object me )
{
        ...
        if( sscanf(arg, "%d,%d,%d", z, x, y) == 3 )
        {
                send_user( get_scene_object(z, x, y, USER_TYPE), "%c%w%w%w%c%c%w%c%c%c", 0x4f, x, y, 
                        42321, 3, OVER_BODY, 42322, 3, UNDER_FOOT, PF_ONCE );
        ...
        send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%w%c%c%w%c%c%c", 0x6f, getoid(who), 
                42321, 1, OVER_BODY, 42322, 1, UNDER_FOOT, 42323, 1, UNDER_FOOT, PF_ONCE );
