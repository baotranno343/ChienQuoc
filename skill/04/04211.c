...
#define THIS_SKILL 0421
#define THIS_PERFORM 04211
#define PF_START "04211"
#define PF_NAME "Biến Thân Thuật"

#define SKILL_LEVEL 35
#define TIME_INTERVAL 45
#define SUB_MP (50 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 17 * 20)
#define ADD_INTERVAL (10 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) / 17 * 1)
...
string get_desc(object me)
{
        ...
        cur_level = (level - SKILL_LEVEL) / 17 + 1;
        result = sprintf(HIC " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 3 lá Bùa Giấy\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Đạo thuật có khả năng biến kẻ địch thành một động vật. Trong hình thể động vật sẽ không thể sử dụng bất cứ phép thuật hay kỹ năng nào (Nhưng có thể sử dụng vật phẩm).Bị biến thân,Né tránh gia tăng 300%%, Thân thủ gia tăng 50%%.\n    Sử dụng với quái vật và Boss không có hiệu quả.\n    Xác xuất biến thân thành công dựa vào cấp bậc của mình và địch nhân.\n    Biến thân thuật hiểu qua trong %d giây.\n " HIC "Cấp tiếp theo:\n " NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Biến thân thuật hiệu quả kéo dài %d giây.",
                         PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_INTERVAL, cur_level * 17 + SKILL_LEVEL, ADD_INTERVAL + 1);
        return result;
}
...
int main(object me, object who, int x, int y, string arg)
{
        ...
        {
                printf(ECHO "Đối với người mới có thể sử dụng \"" PF_NAME "\".");
                return 1;
        }
...
int cast_done(object me)
{
        ...
        {
                /*    if( me->get_weapon_code() == SWORD ) interval = ADD_INTERVAL;
                else if( me->get_weapon_code() == BLADE ) interval = 10;*/
                ...
                if (who->is_user())
                        who->set_save("pf#4243", type);
                else
                        who->set("pf#4243", type);
...
void can_perform(object me)
{
        ...
        if (interval2 < 0)
                ...
                send_user(me, "%c%w%c%w%w%s", 0x50, THIS_PERFORM, get_perform_type(), interval, interval2, PF_NAME);
